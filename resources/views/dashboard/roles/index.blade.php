                                        ...
                                        <div class="tfcl-table-listing">
                                            <div class="table-responsive">
                                                <div class="d-flex justify-content-between p-2">
                                                    <div class="d-flex">
                                                        <span class="result-text"><b>{{ $roles->count() }}</b>
                                                            Results
                                                            found</span>
                                                    </div>
                                                        ...
                                                        @forelse ($roles as $role)
                                                            <tr>
                                                                ...
                                                                <td class="column-controller">
                                                                    @if ($role->id > 5)
                                                                        <a href="{{ route('roles.update.status', [$role->id]) }}"
                                                                            class="btn-action tfcl-dashboard-action-edit text-capitalize"><i
                                                                                class="fa-solid {{ $role->status_id == 1 ? 'fa-circle-check' : 'fa-circle-xmark' }} text-{{ $role->status_id == 1 ? 'success' : 'danger' }}"></i></a>
                                                                    @else
                                                                        <a href="javascript:void(0)" class="btn-action tfcl-dashboard-action-edit text-capitalize disabled"
                                                                            aria-disabled="true" tabindex="-1" title="Status change disabled for system roles"
                                                                    ...
                                                                    @else
                                                                        <a href="javascript:void(0)" class="btn-action tfcl-dashboard-action-edit disabled"
                                                                            aria-disabled="true" tabindex="-1" title="Editing disabled for system roles"
                                                                            style="pointer-events: none; opacity: 0.5; cursor: not-allowed;"><i
                                                                                class="fa-solid fa-pen-to-square"></i></a>
                                                                    @endif
                                                                    @if ($role->id > 5)
                                                                        <form action="{{ route('roles.destroy', $role->id) }}" method="POST" style="display:inline;">
                                                                            @csrf
                                                                            @method('DELETE')
                                                                            <button type="submit" class="btn-action tfcl-dashboard-action-edit" onclick="return confirm('Are you sure you want to delete this role?');" style="background:none;border:none;padding:0;"><i class="fa-solid fa-trash-can"></i></button>
                                                                        </form>
                                                                    @else
                                                                        <a href="javascript:void(0)" class="btn-action tfcl-dashboard-action-edit disabled"
                                                                            aria-disabled="true" tabindex="-1" title="Delete disabled for system roles"
                                                                            style="pointer-events: none; opacity: 0.5; cursor: not-allowed;"><i
